...
def live_find_save(camera_num=0, time_out=20, out_img_path = "./data"):
    ...
    Args:
        ...
        out_img_path (str, optional): output image path. Defaults to "./data".
    """
...
def live_find_transform(camera_num=0, time_out=0, save_image=False, out_img_path="./data",
                        size=(150,150), AR_ids_list=[34,37,10,17], position_num=4):
    """Find AR marker until finding over 4 markers or timeout
        and estimate indicator position and transform the image. 
    Args:
        camera_num (int, optional): camera number. Defaults to 0.
        time_out (int, optional): time out time [s] if time_out=0, no time out. Defaults to 0.
        save_image (bool, optional): save image if find 5 marker. Defaults to False.
        out_img_path (str, optional): save image path. Defaults to "./data".
        size (tuple, optional): size between markers. unit is pixcel Defaults to (150,150).
        AR_ids_list (list, optional): corner AR IDs. order LT->RT->RB->LB Defaults to [34,37,10,17].
        position_num (int, optional): 5th AR ID that is indidcator. Defaults to 4.
    """
            ...
            if ids is not  None:
                ...
                if len(ids) >= 4:
                    img_trans,indicator = image_transform(img=image, corners=corners, ids=ids, 
                                            size=size, AR_ids_list=AR_ids_list, position_num=position_num, 
                                            info=False, image_save=False, ar_cut_position='edge')
